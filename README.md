...
- `QMS_WEB_MODE` - choose the player-facing web runtime: `browser` exposes only the installable app at `/app/`, `server` exposes only the server-owned `/setup` and `/game` flow, and `both` exposes both while landing on the browser app when its bundle is available. The default is `both` for local compatibility; the Cloud Run workflow defaults to `browser`. Browser modes need a bundle, which the Docker image builds; `QMS_BROWSER_DIST_DIR` points at one for a non-Docker run.
