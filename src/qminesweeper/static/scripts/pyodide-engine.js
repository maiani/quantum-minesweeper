...
// (at the py/ root, above the package directories it names). We fetch that
// manifest at boot so there is a single
...
class PyodideEngine {
  // pyBaseURL: where the Python packages are served from. Manifest entries are
  // package-qualified (e.g. "qminesweeper/board.py", "chppy/tableau.py"), so
  // this points at the directory *above* the packages, not into one of them.
  ...
  constructor({ pyBaseURL = "/py/", indexURL = null, cacheBust = null, onProgress = null } = {}) {
  ...
  async _boot() {
    ...
    // Read the build-emitted module list, then fetch each module it names. The
    // names are package-relative paths, mirrored under /lib (already on
    // Pyodide's sys.path), so both qminesweeper and chppy become importable.
    ...
    await Promise.all(
      modules.map(async (name) => {
        ...
        const destination = "/lib/" + name;
        // Create the package directory before writing into it. mkdirTree is
        // idempotent, so concurrent fetches sharing a package are fine.
        pyodide.FS.mkdirTree(destination.slice(0, destination.lastIndexOf("/")));
        pyodide.FS.writeFile(destination, await res.text());
        loaded += 1;
        this._emit(
