...
class QuantumBackend(ABC):
    ...
    def random_clifford_circuit(self, k: int, *, seed: Optional[int] = None) -> List[Tuple[str, List[int]]]:
        ...
        ``seed`` is **best-effort and per-backend**, not a portable guarantee.
        Every implementation must accept the keyword so callers can pass it
        uniformly. A backend that honours it gives in-backend reproducibility:
        the same seed and ``k`` return the same circuit from that backend. One
        whose sampler exposes no seeding API accepts and ignores it. A seed is
        therefore not a cross-backend board identifier. See "Simulator backends"
        in docs/architecture.md for which backends honour it.
        """
        ...
